...
function handleGlobalEvent(event) {
	...
	// The inline-edit Babel plugin tags <Link> with data-edit-id. With edit mode on, those
	// clicks would hit findEditableElementAtPoint and open the editor instead of routing.
	// Never steal events from app chrome (same regions as selection-mode exclusions).
	...
	// When edit mode is on but the click is not on an editable element, do not block:
	// the previous else branch used preventDefault/stopPropagation on every click,
	// which broke SPA navigation (sidebar, buttons, links).
}
